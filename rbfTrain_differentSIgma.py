...
def calcSignma(centers):
	...
	# print dist.shape, np.amax(dist, axis = 1).shape
	return np.amax(dist, axis = 1)/np.sqrt(num_centroids)
...
def trainRBF(train_data, train_label, num_centroids, num_output):
	...
	centroids = sklearn.cluster.k_means(train_data, init='random', n_clusters=num_centroids, max_iter = 100, n_jobs=-1)
	# print centroids[0].shape, centroids[1].shape
	...
	print 'Train Data ', train_data.shape,'Sigma ',sigma.shape, 'phi ', phiMatrix.shape, 'D ',dMatrix.shape, 'Weights ', weights.shape
